...
# Days in each month (non-leap year) for day of year calculation
days_in_month_prefix_sum = [0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334]

def date_format_check(startdate_str):
    """Checks if the given date string matches DD-MM-YYYY format."""
    pattern = r"^\d{2}-\d{2}-\d{4}$"
    return bool(re.match(pattern, startdate_str))

def get_day_of_year(day, month, year):
    """Calculates the day of the year for a given date, accounting for leap years."""
...
def gmst(julian_date):
    """Calculates Greenwich Mean Sidereal Time (GMST) for a given Julian date."""
...
def teme_to_ecef(teme_pos, julian_date):
    """Converts a position vector from TEME to ECEF coordinates."""
...
def geodetic_to_cartesian_ecef(lat_deg, lon_deg, alt_km):
    """Converts geodetic coordinates to ECEF Cartesian coordinates."""
    a = 6378.137
    f = 1 / 298.257223563
    e2 = 2 * f - f**2
...
def lat_change(distance_km):
    """Calculates latitude change for a given distance on Earth's surface."""
    earth_radius_km = 6371
    return (distance_km / earth_radius_km) * (180 / math.pi)

def lon_change(lat_deg, distance_km):
    """Calculates longitude change for a given distance at a specific latitude."""
    lat_rad = math.radians(lat_deg)
...
def get_scanning_square_corners(sat_lat, sat_lon, scan_half_width_km=100):
    """Calculates the corners of a square scanning area centered at satellite's sub-point."""
    try:
...
def is_point_in_scan_area(target_lat, target_lon, scan_box):
    """Checks if a target point is within the satellite's scanning square."""
    try:
...
def calculate_orbit_data(tle1, tle2, start_date_str, rate_choice, requested_time_h_float, start_from_tle_date, target_lat, target_lon, live_only=False):
    """Calculates satellite orbit positions and coverage data, or only live position if live_only=True."""
    ...
    # Calculate live satellite position
    ...
    for i in range(num_samples + 1):
        ...
        if error_code == 0:
            ...
            scan_box_corners = get_scanning_square_corners(current_sat_lat, current_sat_lon)
...
def prepare_3d_scan_box_data(orbit_data_all_points):
    """Prepares scan box corner data (geodetic) for 3D visualization."""
    scan_boxes_3d = []
...
def generate_3d_globe_html(orbit_data_geodetic, covered_points_geodetic, target_point_geodetic,
                           scan_boxes_geodetic, live_sat_data_full, show_scan_boxes_js):
    """
    Generates HTML for a 3D globe that renders all elements from geodetic coordinates,
    including a custom satellite object with live updates.
    """
    ...
    <body>
        <script>
            // --- DATA FROM PYTHON (geodetic coordinates) ---
            ...
            const liveSatDataFull = {json.dumps(live_sat_data_full, default=str)};
            ...
            const satelliteRedDotMaterial = new THREE.MeshBasicMaterial({{ color: 0xff0000 }});
            ...
            function createSatelliteMesh() {{
                ...
                body.add(panel2);
                const redDotGeometry = new THREE.SphereGeometry(10, 16, 16);
                const redDot = new THREE.Mesh(redDotGeometry, satelliteRedDotMaterial);
                redDot.position.set(0, 0, 25);
                body.add(redDot);
            ...
            function updateLiveSatellite(lat, lon, alt, time_str) {{
                ...
                liveSatelliteObject.position.copy(latLonAltToVector3(lat, lon, alt));
...
def generate_2d_map_html(orbit_data_all_points, requested_time_h_float, covered_orbit_positions_with_time_and_box, target_point_geodetic, live_sat_pos_geodetic):
    """Generates HTML for a 2D Leaflet map with live satellite updates."""
        ...
        <script>
            ...
            function updateLiveSatellite(lat, lon, alt, time) {{
                ...
                liveMarker = L.marker([lat, lon], {{
                    icon: L.divIcon({{className: 'fas fa-satellite', html: '<i class="fas fa-satellite" style="color:red;font-size:16px;"></i>'}})
                }}).bindPopup(`<b>LIVE Satellite</b><br>Time: ${{time}}<br>Lat: ${{lat.toFixed(2)}}, Lon: ${{lon.toFixed(2)}}<br>Alt: ${{alt.toFixed(2)}} km`)
                  .addTo(map);
                console.log('Live satellite marker added at:', {{lat, lon, alt, time}});
            }}
